...
static int xhci_mtk_clks_enable(struct xhci_hcd_mtk *mtk)
{
	...
	ret = clk_prepare_enable(mtk->free_clk);
	if (ret) {
		dev_err(mtk->dev, "failed to enable free_clk\n");
		goto free_clk_err;
	}
...
static void xhci_mtk_quirks(struct device *dev, struct xhci_hcd *xhci)
{
	...
	*/
	...
	xhci_dbg(xhci, "%s: XHCI_MTK_HSCH_CFG1:0x%x\n", __func__, readl(hcd->regs+XHCI_MTK_HSCH_CFG1));
}
...
static int xhci_mtk_probe(struct platform_device *pdev)
{
	...
	mtk->free_clk = devm_clk_get(dev, "free_ck");
