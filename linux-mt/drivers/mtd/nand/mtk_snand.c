...
#define NAND_MAX_OOBSIZE 218 /* [Note: Bayi]fix me */
...
static const snand_flashdev_info gen_snand_FlashTable[] = {
	{{0xEF, 0xAA}, 2, 128, 128, 2048, 64, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "Winbond 1Gb", 0x00000000},
	{{0xEF, 0xAB}, 2, 256, 128, 2048, 64, 0x00000000, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "Winbond 2Gb", 0x00000000},
	{{0xC8, 0xD4}, 2, 512, 128, 2048, 64, 0x00000000, 0x00000000, 0x1A00001A,
	...
	{{0x98, 0xCD}, 2, 512, 128, 2048, 64, 0x00000000, 0x00000000, 0x1A00001A,
	...
	{{0xD5, 0x03}, 2, 512, 128, 2048, 64, 0x00000000, 0x00000000, 0x1A00001A,
...
static void mtk_snand_dev_enable_spiq(struct mtk_snfc *snfc, bool enable)
{
	...
	snfi_writel(snfc, 0, NFI_SNAND_MAC_OUTL);
				      ...
				      u32 u4PageAddr)
{
	...
	if ((u4ErrorNumDebug0 & 0x000FFFFF) != 0
		...
		for (i = 0; i <= u4SecIndex; ++i) {
			...
			u4ErrNum &= 0x1F;
			if (u4ErrNum == 0x1F) {
				pr_warn("ECC-U, Page =%d, Sector id=%d\n",
					u4PageAddr, i);
				failed_sec++;
			} else {
				if (u4ErrNum) {
					correct_count += u4ErrNum;
					pr_debug("ECC-C, #err:%d, PA=%d, S=%d\n",
		...
		if (ret == 0)
			if (is_empty_page(spareBuf, sec_num)
			    && return_fake_buf(pDataBuf, page_size, sec_num,
			    u4PageAddr)) {
				ret = 1;
...
static bool mtk_snand_dma_read_data(struct mtd_info *mtd, u8 *buf, u32 num_sec)
{
	...
	if (interrupt_en) {
		...
		while (num_sec > ((snfi_readw(snfc, NFI_BYTELEN) & 0x1f000)
		       >> 12)) {
			timeout--;
			if (timeout == 0) {
...
{
	memset(local_oob_buf, 0xFF, mtd->oobsize);
	if (mtk_snand_read_oob_raw(mtd, local_oob_buf, page, mtd->oobsize/2)
				   == 0) {
			      ...
			      struct mtk_snand_host *host)
{
	...
	snfi_writew(snfc, 0x1, NFI_PAGEFMT);
...
static int mtk_snand_probe(struct platform_device *pdev)
{
	...
	if (!g_bmt) {
		#if defined(MTK_COMBO_NAND_SUPPORT)
		g_bmt = init_bmt(nand_chip, ((PART_SIZE_BMTPOOL)
				>> nand_chip->phys_erase_shift));
		#else
		g_bmt = init_bmt(nand_chip, BMT_POOL_SIZE);
		#endif
		if (!g_bmt) {
			dev_warn(dev, "Error: init bmt failed\n");
			return 0;
		}
	}
