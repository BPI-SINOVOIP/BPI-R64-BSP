...
int fe_qdma_rx_dma_init(struct net_device *dev)
{
	...
	for (i = 0; i < NUM_QRX_DESC; i++) {
		ei_local->netrx0_skbuf[i] =
		    netdev_alloc_skb(dev, MAX_RX_LENGTH + NET_IP_ALIGN);
...
int ei_qdma_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case RAETH_QDMA_REG_READ:
		...
		if (ei_local->chip_name == MT7622_FE) {	/* harry */
			...
			/* pr_debug("page=%d, reg.off =%x\n", page, reg.off); */
		}

		reg.val = sys_reg_read(QTX_CFG_0 + reg.off);
		/* pr_err("read reg off:%x val:%x\n", reg.off, reg.val); */
