...
void set_r50(u8 port_num, u32 r50_cal_result)
{
	int rg_zcal_ctrl_tx, rg_zcal_ctrl_rx;
	u16 l4r22_temp;
...
void fe_cal_r50(u8 port_num, u32 delay)
{
	...
	virt_addr = ioremap(MT7622_CHIP_ID, 0x10);
...
/*Yiron default setting*/
	...
	while ((fe_cal_r50_flag == 0) && (cnt < 0x03)) {
		...
		} else if (reg_value == 0xCB00) {
			fe_cal_r50(port_num, 1);
		} else {
			pr_info("MT722 chip id error\n");
		}
